...
def retrieval_eval(model, ref_model, t_models, t_ref_models, t_test_transforms, data_loader, tokenizer, t_tokenizers,
    ...
    img_attacker = ImageAttacker(model, images_normalize, model_name='', epsilon=8 / 255, alpha=6 / 255, epoch=10,
                                 decay=1.,
    ...
    attacker = Attacker(model, img_attacker, txt_attacker, 10)
    ...
    ### attack and store adversarial imgs, txts for each batch ###
    print('Forward')
    for batch_idx, (img_names, images, texts_group, images_ids, text_ids_groups) in enumerate(data_loader):
        ...
        # generate adversarial images and texts
        adv_images, adv_texts = attacker.attack(images, texts, txt2img, device=device,
                                                max_length=max_length, scales=scales)


        # store adv images, texts - KSE modified
        if args.store:
            adv_path = f'result/{args.exp_name}'
            os.makedirs(f'result/{args.exp_name}' + '/adv_imgs', exist_ok=True)

            for name, img in zip(img_names, adv_images):
                img = img.cpu().detach().numpy().transpose(1, 2, 0) * 255.0
                img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
                name = name.split('/')[-1]
                cv2.imwrite(os.path.join(adv_path + f'/adv_imgs/{name}.png'), img)

            with open(adv_path + 'adv_txts.txt', 'a') as f:
                for i, txt in enumerate(adv_texts):
                    for t in txt:
                        f.write(f'{t} ')
                    f.write('\n')

        # store adversarial features
        with torch.no_grad():
...
def eval_asr(model, ref_model, tokenizer, t_models, t_ref_models, t_tokenizers, t_test_transforms, data_loader, device,
    ...
    result_file_path = f"./result/{args.exp_name}"          # KSE modified
    os.makedirs(result_file_path, exist_ok=True)

    with open(result_file_path+'/result.txt', "a") as file:
        file.write("\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', default='Retrieval_coco')     # KSE modified
    parser.add_argument('--seed', default=42, type=int)
    parser.add_argument('--batch_size', default=20, type=int)
    parser.add_argument('--cuda_id', default=0, type=int)
    parser.add_argument('--store', default=False, type=bool)

    parser.add_argument('--model_list', default=['ALBEF', 'TCL', 'CLIP_ViT', 'CLIP_CNN'], nargs='+')
    ...
    parser.add_argument('--albef_ckpt', default='checkpoints/ALBEF_mscoco.pth', type=str)
    parser.add_argument('--tcl_ckpt', default='checkpoints/TCL_Retrieval_Coco_Finetune.pth', type=str)
