...
class paired_dataset(Dataset):
    def __init__(self, ann_file, transform, image_root, max_words=30):
        ...
        self.text = []      # save captions
        self.image = []     # save image path

        self.txt2img = {}   # img index per a text caption {0:0, 1:0, 2:0, 3:0, ...}
        self.img2txt = {}   # text indexes per an image {0:[0, 1, 2, 3, 4], 1:[5, 6, 7, 8, 9], ...}
    ...
    def __getitem__(self, index):
        ...
        return self.image[index], image, texts, index, text_ids
        # return image, texts, index, text_ids

    def collate_fn(self, batch):
        img_names, imgs, txt_groups, img_ids, text_ids_groups = list(zip(*batch))
        # imgs, txt_groups, img_ids, text_ids_groups = list(zip(*batch))
        imgs = torch.stack(imgs, 0)
        return img_names, imgs, txt_groups, list(img_ids), text_ids_groups
