...
from GLEAM.Attacks.DRAttacker import Attacker, ImageAttacker, TextAttacker
...
def retrieval_eval(model, ref_model, t_models, t_ref_models, t_test_transforms, data_loader, tokenizer, t_tokenizers, device, args,config):
    ...
    for batch_idx, (images, texts_group, images_ids, text_ids_groups,image_paths) in enumerate(data_loader):
        ...
        # adv_images, adv_texts, execuate_time = attacker.attack(images, texts, txt2img, all_texts, device=device, max_length=max_length, scales=scales)
        adv_images, adv_texts, execuate_time = attacker.attack(images, texts, txt2img, device=device, max_length=max_length, scales=scales)
    ...
    return s_score_matrix_i2t, s_score_matrix_t2i, t_score_matrix_i2ts, t_score_matrix_t2is
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', default='temp')
    ...
    parser.add_argument('--albef_ckpt', default='./checkpoints/ALBEF.pth', type=str)
    parser.add_argument('--tcl_ckpt', default='./checkpoints/TCL_Retrieval_Flickr_Finetune.pth', type=str)
