...
class Attacker():
    ...
    def attack(self, imgs, txts, txt2img, device='cpu', max_length=30, scales=None, **kwargs):
        ...
        adv_txts = self.txt_attacker.img_guided_attack(self.model, txts, img_embeds=img_supervisions)  # 1차 텍스트 공격
        ...
        with torch.no_grad():
            ...
            txt_supervisions = txts_output['text_feat']  # 공격한 text의 embedding
        start_time = time.time()
        adv_imgs = self.img_attacker.txt_guided_attack(self.model, imgs, txt2img, device,
                                                       scales=scales,
                                                       txt_embeds=txt_supervisions)  # 공격한 text로 image attack
        end_time = time.time()
        execuate_time = end_time - start_time

        with torch.no_grad():
            adv_imgs_outputs = self.model.inference_image(self.img_attacker.normalization(adv_imgs))
            img_supervisions = adv_imgs_outputs['image_feat'][txt2img]  # 공격한 image의 embedding
        adv_txts = self.txt_attacker.img_guided_attack(self.model, txts, img_embeds=img_supervisions)  # 최종 adv text 생성
